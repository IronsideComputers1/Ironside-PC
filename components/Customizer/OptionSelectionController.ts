...
const OptionSelectionController = ({
  product,
        ...
        data?.node?.values?.edges.forEach((product: any) => {
          ...
          if (colorOpts) {
              ...
              setSelectedColor((prev: any) => {
                ...
                if (catExist >= 0) {
                  prev[catExist] = {
                    ...
                    parent_id: productId,
                    product_image:
                      colorOpts?.images?.edges[0]?.node?.urlOriginal,
                  }
                }
                if (catExist < 0) {
                  return [...prev, colorOptionData[0]]
                }
                return prev
              })
            ...
            if (selectedColor?.length) {
              const filteredData = selectedColor?.filter(
  ...
  const caseImageChange = (product_id: number, colorOpts: any) => {
